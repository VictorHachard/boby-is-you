...
public class Slip extends Rule {
    ...
    boolean work(Position pos,Directions dir,TypeElement te) {
        ...
        if (isSlip) {
            ...
            if (!(listGrid.get(pos.y+this.dirSlip.getDirVer())
                    .get(pos.x+this.dirSlip.getDirHori()).find(Property.SLIP))){
                //pour sortire de la ICCE
                if ((listGrid.get(pos.y+dirSlip.getDirVer())
                        .get(pos.x+dirSlip.getDirHori()).canAdd())) {
                    //si stop return false
                    board.editPlacement(pos,dirSlip,te);
                    isSlip = false;
                }
                //Si il y a un eleme a push en dehor de ICE
                else if (listGrid.get(pos.y+this.dirSlip.getDirVer())
                        .get(pos.x+this.dirSlip.getDirHori()).canPush()) {
                    //verifie si il peut push la case suivante
                    if (board.push(new Position(pos.x+dirSlip.getDirHori(),
                            pos.y+dirSlip.getDirVer()),dirSlip))              
                        board.editPlacement(pos,dirSlip,te);
                    isSlip = false;
                }
                //si renconter un mur
                else if (!(listGrid.get(pos.y+dirSlip.getDirVer())
                        .get(pos.x+dirSlip.getDirHori()).canAdd())) {
                    //si stop return false
